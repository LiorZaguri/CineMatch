...
export class MovieService {
  ...
  refreshMovies(): Observable<MovieCatalogResponse> {
    ...
    return forkJoin({
      ...
      nowPlaying: this.http.get<TmdbMovieListResponse>(
        `${this.gatewayMovieUrl}/now-playing/?page=1`,
      ),
      ...
      map(({ dashboard, popular, nowPlaying, upcoming, topRated }) => {
        const catalog = {
          dashboard: {
            now_playing: this.mapMovies(
              nowPlaying.results.length > 0 ? nowPlaying.results : dashboard.now_playing,
            ),
            popular: this.mapMovies(
              popular.results.length > 0 ? popular.results : dashboard.popular,
            ),
            upcoming: this.mapMovies(
              upcoming.results.length > 0 ? upcoming.results : dashboard.upcoming,
            ),
            top_rated: this.mapMovies(
              topRated.results.length > 0 ? topRated.results : dashboard.top_rated,
            ),
  ...
  getMovieByTmdbId(tmdbId: string, forceRefresh = false): Observable<Movie> {
    ...
    if (cachedMovie) {
      return this.http
        .get<TmdbMovie & { reviews?: unknown[] }>(`${this.gatewayMovieUrl}/${cachedMovie.id}/`)
        .pipe(map((movie) => this.mapMovie(movie)));
    }

    return this.http
      .get<TmdbMovie & { reviews?: unknown[] }>(`${this.gatewayMovieUrl}/${tmdbId}/`)
      .pipe(map((movie) => this.mapMovie(movie)));
  }
